...
export async function POST(request: Request, response: Response) {
  ...
  console.log(user.data[0].provider?.google?.sync?.googleTimeOff?.value, "user.data[0]");
  ...
  if (
    ...
    let response = await axios.post(
      `https://www.googleapis.com/calendar/v3/calendars/${scopedUser.provider.google.calendarId}/events`,
      {
        summary: body.note,
        start: {
          dateTime: body.timeOffPeriod.period.start,
        },
        end: {
          dateTime: body.timeOffPeriod.period.end,
        },
      },
      {
        headers: {
